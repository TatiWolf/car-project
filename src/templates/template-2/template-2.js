...
`).join('');
  ...
  document.getElementById('points2').innerHTML = data.points2.map((point, index) => {
    if (index + 1 <= 4) {
      return `<div style='display: flex; align-items: flex-start; margin-top: 20px;'><div class='text-wrap mr-20'>${index + 1}</div><li>${point}</li></div>`;
    } else {
      return '';
    }
  }).join('');

  document.getElementById('points22').innerHTML = data.points2.map((point, index) => {
    if (index + 1 > 4) {
      return `<div style='display: flex; align-items: flex-start; margin-top: 20px;'><div class='text-wrap mr-20'>${index + 1}</div><li>${point}</li></div>`;
    } else {
      return '';
  ...
  document.getElementById('points3').innerHTML = data.points3.map(point => `
...
`).join('');
  ...
  document.getElementById('points4').innerHTML = data.points4.map(point => `
<div style='display: flex; align-items: center;' class='mt-20'><img class='mr-20' src='../../assets/img/icon/circle.svg' alt=''><li > ${point}</li></div>
`).join('');
...
let template2 = [
  {
    ...
    points1:
      [
        ...
        'выставить знак аварийной остановки: не менее 15 метров от ДТП в населённых пунктах и не менее 30 метров за городом;',
        'проверить, имеются ли пострадавшие среди остальных участников происшествия;',
        'не перемещать предметы, имеющие отношение к ДТП — осколки фар, части бампера и т.д. — все оставьте как есть.'
      ],
    ...
    points2: [
      'Попросите его паспорт, сфотографируйте документ. Человек вправе отказаться. Тогда возьмите данные из протокола ГИБДД.',
      'Поинтересуйтесь, намерен ли виновник ДТП компенсировать ущерб и в каком размере.',
      'Выясните сроки и порядок компенсации: проще говоря, когда виновник оплатит ремонт.',
      'Человек может сразу же согласиться перевести вам деньги или отдать наличными.',
      'Составьте расписку. Документ пишется в свободной форме, однако важно, чтобы в нем было указано, между кем и кем он составлен (с указанием паспортных данных), дата, причина, размер компенсации и срок возмещения. </br> Теоретически виновник может отказаться платить на месте. Тогда укажите в расписке, до какого времени он обязан перевести деньги на оплату ущерба.',
      'Пострадавший после получения компенсации также пишет расписку о том, что деньги принял, претензий не имеет.'
    ],
    ...
    points3: [
      ...
      'авария никому не причинила ущерб, кроме двух участников ДТП;',
      '  не задета дорожная инфраструктура (столбы, светофоры, заборы), а также в целости личное имущество водителей (смартфоны, прочая техника и вещи);',
      'у участников аварий нет разногласий по поводу обстоятельств ДТП и полученных повреждений;',
      'один из участников аварии в будущем не хочет получить выплату по КАСКО;',
    ...
    points4: [
      ...
      'связаться со своей страховой компанией и сообщить о страховом случае;',
      'убедиться, что другие участники ДТП сделали тоже самое.'
    ],
